...
mtest <- importRaster(fname = "~/Simularia/Progetti/Arianet/Italcementi_Palermo/data/spray/avg_SO2A15_inv.nc", k = 1000, destaggering = T, variable = "SO2A15")
...
test2 <- importRaster(fname = "test/99p_ALL_NOXALL_year.grd", k = 1000, destaggering = T)
levels2 <- c(10, 25, 50, 75, 100, 150, 200, 300, 400)
contourPlot(test2, levels = levels2)
# ggplot(test2, aes(x, y, z = z)) + stat_contour(aes(fill = factor(..level..)), breaks = levels2, geom = "polygon")
# ggplot(test2, aes(x, y, z = z)) + stat_hollow_contour(aes(fill = factor(..level..)), breaks = levels2)

test3 <- importRaster("test/avg_TVMICRO_TDEPDIOX_year.grd", k = 1000)
levels3 <- c(0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1.0)
contourPlot(test3, levels = levels3)
