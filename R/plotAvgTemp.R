...
#' @param avg.time This defines the time period to average to (see openair::timeAverage)
...
plotAvgTemp <- function(mydata, temp="temp", avg.time="1 month", ylabel="Temperatura [°C]") {
    ...
    v <- ggplot(data=mydata_mean, aes(date, temp)) + geom_histogram(aes(date, temp, color="Media", fill="Media"), stat="identity", show_guide=FALSE) + 
