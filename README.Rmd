...
The package is developed and maintained at [Simularia](https://www.simularia.it) and it is widely used for their daily work.
...
For more information about *R* and *Python* interoperability, refer to the [`reticulate`](https://rstudio.github.io/reticulate/) documentation.
...
The plot is customisable by using `contourPlot2()` arguments and by piping **ggplot2** instructions with the `+` operator.

In the following example, the original domain is cropped, colour levels are explicitely assigned and a legend name is provided through function arguments.
Furthermore, `labs()` and `theme_minimal()` functions from **ggplot2** are used to remove axis labels and to change the overall theme:
```{r echo=TRUE}
library(ggplot2)
contourPlot2(
    nox_concentration,
...
In order to save the last plot to file, you can directly use the **ggplot2** function `ggsave()`:
```{r eval=FALSE}
