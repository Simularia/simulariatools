...
- `turnerStabilityClass()` compute atmospheric PGT stability class with Turner method.
...
and it is widely used for their daily work.
...
```
Concentration data are imported as `data.frame` with `x`, `y` and `z` columns
for coordinates and grid values.
```{r echo=TRUE}
...
**ggplot2** instructions with the `+` operator.
